...
class MyTokenManager {
    ...
    public static function createPatientToken($userId){
        ...
        DB::select('insert into user_token(userId,token) VALUES (?, ?)',
        [$userId,$encToken]);
    ...
    public static function currentPatient(Request $request){
        ...
        if($result){
            ...
            if(Hash::check($tokenStr,$tokenData->token)){
                ...
                $queryResult = DB::select('select * from users where id = ?',[$tokenData->userId]);
    ...
    public static function removePatientToken(Request $request){
        ...
        DB::delete('delete from user_token where id = ?',[$tokenId]);
    }
